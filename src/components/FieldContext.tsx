...
export const IsObjectOrArrayAccessible = (obj: any) => {
  return (
    IsNotUndefinedOrNull(obj) && (typeof obj === "object" || Array.isArray(obj))
  );
};
...
export const FieldContextProvider: ParentComponent<
...
> = (props) => {
  ...
  const parentDefaultValue = createMemo(() =>
    parentFieldContext.defaultValue() ?? valid()
      ? undefined
      : props.defaultValue
  );
  ...
  const name = createMemo(() =>
    valid() ? props.name! : parentFieldContext.name
  );
  const path = createMemo(() =>
    valid()
      ? [...parentFieldContext.path, props.name!]
      : parentFieldContext.path
  );
      ...
      if (IsNotUndefinedOrNull(parentValue())) {
        if (IsObjectOrArrayAccessible(parentValue())) {
          ...
          console.warn(
            "cannot get value",
            path(),
            "expect a object or a array, but get",
            parentValue()
          );
        }
      }
  ...
  const setValue = createMemo(() => {
    return (value: any) => {
      ...
      Log("[FIELD CONTEXT]", path(), "setValue", value);
  ...
  return (
    <FieldContext.Provider value={nextContext()}>
      {props.children}
    </FieldContext.Provider>
  );
};

export const FieldInput: ParentComponent<
  JSX.InputHTMLAttributes<HTMLInputElement>
> = (props) => {
  const [local, elProps] = splitProps(props, ["children"]);

  const context = useFieldContext();
  const handleInput: JSX.InputHTMLAttributes<HTMLInputElement>["oninput"] = (
    e
  ) => {
...
export const Border: ParentComponent<JSX.HTMLAttributes<HTMLDivElement>> = (
  props
) => {
  return (
    <div style={{ border: "1px solid #eee", padding: "1rem" }} {...props}></div>
  );
};
...
export const FieldTest: ParentComponent = (props) => {
  ...
  return (
    <div style={{ border: "1px solid #eee", padding: "1rem" }}>
      <p>
        name:<strong>{current.name}</strong> value: {display()}
      </p>
      <p>
        path:<span>{JSON.stringify(current.path)}</span>
      </p>
      {props.children}
    </div>
  );
...
export const FieldList: Component = (props) => {
  ...
  return (
    <Border>
      ...
      <Index each={value()}>
        {(item, index) => (
          <FieldContextProvider name={index}>
